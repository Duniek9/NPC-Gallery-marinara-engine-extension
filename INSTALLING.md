...
3. Enter the full Marinara Engine 2.3.4 folder when prompted. This is the folder containing Marinara's root `package.json`.
...
```bat
install-npc-gallery.bat "C:\path\to\Marinara-Engine-2.3.4"
```
...
Run the installer first on an unmodified Marinara 2.3.4 installation. The backup captures whatever files exist at installation time. This repository's current development Engine has already been integrated manually, so running the installer there now would treat those integrated files as its original baseline.

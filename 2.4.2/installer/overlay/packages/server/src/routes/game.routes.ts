...
import {
  collectCapabilityGameSetupContext,
  notifyCapabilityGameSetupCompleted,
} from "../services/capability-packages/game-setup-observer.service.js";
...
export async function gameRoutes(app: FastifyInstance) {
  ...
  app.post("/setup", async (req, reply) => {
    ...
    const setupActiveAgentIds = Array.isArray(meta.activeAgentIds)
      ...
      : [];
    const capabilitySetupContext = await collectCapabilityGameSetupContext(setupActiveAgentIds, {
      chatId,
      chatName: chat.name,
    ...
    const messages: ChatMessage[] = [
      {
        ...
        content: [
          ...
          ...capabilitySetupContext.map(
            (section) => `<capability_setup_context>\n${section}\n</capability_setup_context>`,
          ),
        ].join("\n\n"),
      },
      {
    ...
    notifyCapabilityGameSetupCompleted(setupActiveAgentIds, {
      chatId,
      chatName: chat.name,
      partyCharacterNames: partyNames,
      startingNpcs: Array.isArray(setupData.startingNpcs)
  ...
  app.post("/setup/apply-json", async (req, reply) => {
    ...
    for (const characterId of setupConfig.partyCharacterIds) {
      ...
      try {
        ...
        // A malformed party card must not prevent the repaired world setup from completing.
      }
    }
    notifyCapabilityGameSetupCompleted(setupActiveAgentIds, {
      chatId,

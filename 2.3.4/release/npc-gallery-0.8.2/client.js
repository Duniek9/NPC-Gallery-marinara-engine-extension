...
var v = class extends HTMLElement {
	...
	renderReviewModal() {
		...
		for (let t of e.updates) {
			...
			e.style.cssText = "margin-top: 0.625rem; border: 1px solid var(--border); border-radius: 0.625rem; background: var(--card); padding: 0.75rem";
			let r = document.createElement("strong");
			r.textContent = `${t.action === "create" ? "Create" : "Update"}: ${t.name}`, r.style.cssText = "font-size: 0.8125rem";
			let i = document.createElement("div");
			i.style.cssText = "margin-top: 0.5rem; display: grid; gap: 0.375rem; color: var(--muted-foreground); font-size: 0.6875rem; white-space: pre-wrap";
			for (let [e, n] of [
				["Appearance", t.appearance],
				["Personality", t.personality],
				["Description", t.description]
				...
				t.style.cssText = "margin: 0", t.textContent = `${e}: ${n}`, i.append(t);
			}
			e.append(r, i), n.append(e);
		}
		let a = document.createElement("div");
		a.style.cssText = "position: sticky; bottom: -1rem; display: flex; flex-wrap: wrap; gap: 0.5rem; margin: 1rem -1rem -1rem; border-top: 1px solid var(--border); background: var(--background); padding: 0.75rem 1rem";
		let o = (t, n) => {
			let r = document.createElement("button");
			return r.type = "button", r.textContent = t, r.style.cssText = `border: 1px solid var(--border); border-radius: 0.5rem; background: ${n === "accept" ? "var(--primary)" : "var(--secondary)"}; color: ${n === "accept" ? "var(--primary-foreground)" : "var(--foreground)"}; padding: 0.5rem 0.75rem; font-size: 0.75rem; font-weight: 600; cursor: pointer`, r.addEventListener("click", () => n ? void this.resolvePendingExtraction(e.id, n) : (this.reviewModalOpen = !1, this.render(), void 0)), r;
		};
		return a.append(o("Approve", "accept"), o("Reject", "reject"), o("Review later")), n.append(a), t.append(n), t;
	}
	renderGallery() {

...
async function I(r) {
	let { id: c, version: l } = r.package, d = E(r.dataDir), p = D(r.dataDir), g = P(r.dataDir), C = F(r.dataDir), I = N(r.dataDir), L = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), R = Promise.resolve(), z = async (t, n) => {
		...
		let f = c.slice(-20).map((e) => ({
		...
		})), h = await (await r.api.runtime.languageModels.resolve(i.connectionId)).chatComplete([{
			role: "system",
			...
			temperature: .1,
			maxTokens: 2400
		}), _ = h.content ? O(r.api.runtime.json.parseJsonish(h.content)) : [];
		if (a === "automatic") {
	...
	}), H = r.api.registerGameSetupCompletedObserver(async (e) => {
		...
		for (let r of e.startingNpcs) {
			...
			let a = m(r.description, f.description) ?? "", o = A(await p.list(), i);
			if (o) {
				await p.update(o.id, {
					name: o.name,
					aliases: o.aliases,
					...
					description: ((e, t, n) => t && !o.lockedFields.includes(e) ? t : n)("description", a, o.description),
					lockedFields: o.lockedFields,
					sourceChatIds: Array.from(/* @__PURE__ */ new Set([...o.sourceChatIds, e.chatId])),
					lastSeenAt: (/* @__PURE__ */ new Date()).toISOString(),
					folderId: o.folderId ?? n.id
				}), await g.include(e.chatId, o.id);
				continue;
			}
			let s = await p.create({
				name: i,
				aliases: [],
				avatar: null,
				appearance: "",
	...
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		try {
			...
			return !e || e.mode !== "roleplay" || !i ? t.status(400).send({ error: "This Roleplay has no assistant message to analyze yet" }) : (await z({
				chatId: n,
				messageId: i.id,
				swipeIndex: 0,
				mode: "roleplay",
		...
		B(), V(), H(), r.api.runtime.logger.info("Deactivated capability package %s@%s", c, l);
	};
}
async function L(e) {

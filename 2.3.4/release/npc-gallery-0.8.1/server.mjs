...
async function M(r) {
	let { id: c, version: l } = r.package, d = E(r.dataDir), f = D(r.dataDir), p = A(r.dataDir), m = j(r.dataDir), g = k(r.dataDir), C = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), M = Promise.resolve(), N = async (t, n) => {
		...
		n || await g.add(o);
	}, P = r.api.registerAssistantMessageObserver((e) => {
		M = M.then(() => N(e)).catch((e) => {
	...
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		try {
			...
			return !e || e.mode !== "roleplay" || !i ? t.status(400).send({ error: "This Roleplay has no assistant message to analyze yet" }) : (await N({
				chatId: n,
				messageId: i.id,
				swipeIndex: 0,
				mode: "roleplay",
		...
		P(), r.api.runtime.logger.info("Deactivated capability package %s@%s", c, l);
	};
}
async function N(e) {
...
//#endregion
export { M as activate, N as selfCheck };

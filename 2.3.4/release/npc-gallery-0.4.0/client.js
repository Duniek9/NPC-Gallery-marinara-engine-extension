...
var v = class extends HTMLElement {
	...
	renderGallery() {
		...
		a("all", "All NPCs", this.npcs.length), a("unfiled", "Unfiled", this.npcs.filter((e) => !e.folderId).length);
		for (let e of this.folders) a(e.id, e.name, this.npcs.filter((t) => t.folderId === e.id).length, e);
		let o = document.createElement("input");
		o.type = "search", o.placeholder = "Search NPCs…", o.value = this.npcSearch, o.style.cssText = "border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); color: var(--foreground); padding: 0.625rem 0.75rem", o.addEventListener("input", () => {
			this.npcSearch = o.value, this.render();
			let e = this.querySelector("input[type=\"search\"]");
			e?.focus(), e?.setSelectionRange(e.value.length, e.value.length);
		});
		let s = document.createElement("button");
		s.type = "button", s.textContent = "+ Create NPC", s.style.cssText = "border: 0; border-radius: 0.5rem; background: var(--primary); color: var(--primary-foreground); padding: 0.625rem; font-weight: 600; cursor: pointer", s.addEventListener("click", () => this.createNpcDraft());
		let c = document.createElement("div");
		c.style.cssText = "display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem";
		let l = document.createElement("label");
		l.textContent = "Import PNG", l.style.cssText = "display: flex; align-items: center; justify-content: center; border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); padding: 0.5rem; font-size: 0.6875rem; font-weight: 600; cursor: pointer";
		let u = document.createElement("input");
		u.type = "file", u.accept = "image/png,.png", u.style.display = "none", u.addEventListener("change", () => {
			let e = u.files?.[0];
			e && this.importNpcCard(e);
		}), l.append(u);
		let d = document.createElement("button");
		d.type = "button", d.textContent = "Export PNG", d.disabled = !this.npcDraft?.id, d.style.cssText = "border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); color: var(--foreground); padding: 0.5rem; font-size: 0.6875rem; font-weight: 600; cursor: pointer", d.addEventListener("click", () => void this.exportNpcCard()), c.append(l, d), t.append(o, s, c);
		let f = this.npcSearch.trim().toLocaleLowerCase(), p = this.npcs.filter((e) => (this.selectedFolder === "all" || this.selectedFolder === "unfiled" && !e.folderId || e.folderId === this.selectedFolder) && (!f || e.name.toLocaleLowerCase().includes(f) || e.aliases.some((e) => e.toLocaleLowerCase().includes(f))));
		if (p.length === 0) {
		...
		if (m.style.cssText = "min-width: min(100%, 18rem); flex: 3 1 30rem; border: 1px solid var(--border); border-radius: 0.75rem; background: var(--background); padding: 1rem", !this.npcDraft) {
			let n = document.createElement("p");
			return n.textContent = "Select an NPC to edit, or create a new one.", n.style.cssText = "margin: 0; color: var(--muted-foreground); font-size: 0.875rem", m.append(n), e.append(t, m), e;
		}
		let h = (e, t, n = !1) => {
			...
			}), r.append(i), m.append(r);
		}, g = document.createElement("h3");
		if (g.textContent = this.npcDraft.id ? "Edit NPC" : "Create NPC", g.style.cssText = "margin: 0 0 1rem; font-size: 1rem", m.append(g), this.draftRestored || this.draftDirty) {
			let e = document.createElement("p");
			e.textContent = this.draftRestored ? "Unsaved draft restored from this browser session." : "Unsaved changes are preserved while you visit other Marinara panels.", e.style.cssText = "margin: -0.5rem 0 1rem; border: 1px solid color-mix(in srgb, var(--primary) 35%, var(--border)); border-radius: 0.5rem; background: color-mix(in srgb, var(--primary) 8%, transparent); padding: 0.625rem; color: var(--muted-foreground); font-size: 0.6875rem", m.append(e);
		}
		h("Name", "name"), h("Aliases (comma separated)", "aliases"), h("Avatar URL or image value", "avatar");
		let _ = document.createElement("label");
		_.style.cssText = "display: inline-flex; align-items: center; gap: 0.5rem; margin: -0.25rem 0 0.75rem; border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); padding: 0.5rem 0.75rem; font-size: 0.75rem; font-weight: 600; cursor: pointer", _.textContent = this.uploadingAvatar ? "Uploading avatar…" : "Choose image from disk";
		let v = document.createElement("input");
		if (v.type = "file", v.accept = "image/png,image/jpeg,image/gif,image/webp", v.disabled = this.uploadingAvatar || this.saving, v.style.display = "none", v.addEventListener("change", () => {
			let e = v.files?.[0];
			e && this.uploadAvatar(e);
		}), _.append(v), m.append(_), this.npcDraft.avatar) {
			let e = document.createElement("img");
			e.src = this.npcDraft.avatar, e.alt = `${this.npcDraft.name || "NPC"} avatar preview`, e.style.cssText = "display: block; width: 8rem; height: 8rem; margin: 0 0 0.75rem; border: 1px solid var(--border); border-radius: 0.75rem; object-fit: cover; background: var(--muted)", m.append(e);
		}
		let y = document.createElement("label");
		y.style.cssText = "display: flex; flex-direction: column; gap: 0.375rem; margin-bottom: 0.75rem; font-size: 0.75rem; font-weight: 600", y.append("Folder");
		let b = document.createElement("select");
		b.style.cssText = "border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); color: var(--foreground); padding: 0.625rem 0.75rem";
		let x = document.createElement("option");
		x.value = "", x.textContent = "Unfiled", b.append(x);
		for (let e of this.folders) {
			let t = document.createElement("option");
			t.value = e.id, t.textContent = e.name, b.append(t);
		}
		b.value = this.npcDraft.folderId ?? "", b.addEventListener("change", () => {
			this.npcDraft && (this.npcDraft.folderId = b.value || null, this.persistNpcDraft());
		}), y.append(b), m.append(y), h("Appearance", "appearance", !0), h("Personality", "personality", !0), h("Description", "description", !0);
		let S = document.createElement("fieldset");
		S.style.cssText = "display: flex; flex-wrap: wrap; gap: 0.625rem; margin: 0 0 1rem; border: 1px solid var(--border); border-radius: 0.5rem; padding: 0.75rem";
		let C = document.createElement("legend");
		C.textContent = "Protect from AI updates", C.style.cssText = "padding: 0 0.375rem; font-size: 0.6875rem; font-weight: 600", S.append(C);
		for (let e of [
			"name",
			"avatar",
			"appearance",
			...
			}), t.append(n, e), S.append(t);
		}
		m.append(S);
		let w = document.createElement("div");
		w.style.cssText = "display: flex; flex-wrap: wrap; justify-content: space-between; gap: 0.5rem";
		let T = document.createElement("button");
		T.type = "button", T.textContent = "Delete", T.disabled = !this.npcDraft.id || this.saving, T.style.cssText = "border: 1px solid var(--destructive); border-radius: 0.5rem; background: transparent; color: var(--destructive); padding: 0.5rem 0.75rem; cursor: pointer", T.addEventListener("click", () => void this.deleteNpc());
		let E = document.createElement("button");
		E.type = "button", E.textContent = "Discard draft", E.disabled = this.saving, E.style.cssText = "border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); color: var(--foreground); padding: 0.5rem 0.75rem; cursor: pointer", E.addEventListener("click", () => this.discardNpcDraft());
		let D = document.createElement("button");
		D.type = "button", D.textContent = this.saving ? "Saving…" : "Save NPC", D.disabled = this.saving, D.style.cssText = "border: 0; border-radius: 0.5rem; background: var(--primary); color: var(--primary-foreground); padding: 0.5rem 0.75rem; font-weight: 600; cursor: pointer", D.addEventListener("click", () => void this.saveNpc());
		let O = document.createElement("span");
		return O.style.cssText = "display: flex; flex-wrap: wrap; gap: 0.5rem", O.append(T, E), w.append(O, D), m.append(w), e.append(t, m), e;
	}
	...
	render() {
		...
		T.textContent = this.errorMessage, T.style.cssText = "margin: 0.5rem 0 0; color: var(--destructive); font-size: 0.75rem", t.append(n, i), this.activeView === "gallery" ? t.append(this.renderGallery()) : t.append(a, o, u, g), this.deletingFolder && t.append(this.renderFolderDeleteModal()), this.errorMessage && t.append(T), this.append(t);
	}
};

...
async function I(r) {
	let { id: c, version: l } = r.package, d = E(r.dataDir), f = D(r.dataDir), p = P(r.dataDir), m = F(r.dataDir), g = N(r.dataDir), C = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), k = Promise.resolve(), I = async (t, n) => {
		...
		n || await g.add(o);
	}, L = r.api.registerAssistantMessageObserver((e) => {
		k = k.then(() => I(e)).catch((e) => {
	...
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		try {
			...
			return !e || e.mode !== "roleplay" || !i ? t.status(400).send({ error: "This Roleplay has no assistant message to analyze yet" }) : (await I({
				chatId: n,
				messageId: i.id,
				swipeIndex: 0,
				mode: "roleplay",
		...
		try {
			...
			if (s === "regenerate" && h.length > 0) {
				...
				t = j(s.content ? O(r.api.runtime.json.parseJsonish(s.content)) : [], h, o);
			}
		...
		L(), r.api.runtime.logger.info("Deactivated capability package %s@%s", c, l);
	};
}
async function L(e) {

...
export function mergePromptPrefix(prefix, prompt) {
    ...
    if (promptAlreadyStartsWithPrefix(trimmedPrompt, trimmedPrefix))
        return trimmedPrompt;
...
function promptAlreadyStartsWithPrefix(prompt, prefix) {
    ...
    if (prefixFragments.length === 0)
        return false;
    const promptFragments = promptPrefixFragments(prompt);
    if (promptFragments.length < prefixFragments.length)
        return false;
...
function readNullableString(value, fallback) {

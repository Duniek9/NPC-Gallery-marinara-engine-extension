...
export function compileImagePrompt(input) {
    ...
    if (!generatedStyle && !userPositive)
        return initial;
...
function compileImagePromptPass(input, protectGeneratedStyle, protectUserPositive) {
    const profile = findImageStyleProfile(input.styleProfiles, input.styleProfileId || input.imageDefaults?.styleProfileId || input.styleProfiles.defaultProfileId);
    ...
    const profileStyleText = compactPrompt || (profile.styleText && generatedStyle)
        ? ""
    ...
    for (const part of negativeParts) {
        negativeFragments.push(...splitPromptFragments(part, promptMode)
    ...
    let positive = compactPromptFragments(dedupeFragments([...hardPrefix, ...positiveFragments], profile.rules.dedupeStrength, positiveDiagnostics), compactPrompt, hardPrefix.length);
    if (positive.length === 0) {
        positive = fallbackPositiveFragments([generatedStylePart, input.prompt, userPositivePart], promptMode, compactPrompt);
    }
...
function resolveImagePromptCompilationMode(input, profile) {
    const promptMode = profile.promptMode;
    ...
    return {
        ...
        compactPrompt,
        fragmentMode: compactPrompt ? "tagged" : promptMode,
    };
}
function omitPromptValueAlreadyPresent(value, prompt, promptMode, diagnostics) {
    if (!value || !promptContainsPositiveNormalizedValue(prompt, value, promptMode))
        return value;
    diagnostics.push(value);
    return "";
}
function promptContainsPositiveNormalizedValue(prompt, value, promptMode) {
...
function normalizePromptContainmentText(value) {
    return value
        .toLowerCase()
        .replace(/[^\p{L}\p{N}]+/gu, " ")
        .replace(/\s+/g, " ")
...
function fallbackPositiveFragments(values, promptMode, compactPrompt) {
...
function imagePromptPrefixFromDefaults(defaults) {
    if (!defaults)
        return "";
    if (defaults.service === "automatic1111")
        return defaults.automatic1111?.promptPrefix ?? "";
    if (defaults.service === "comfyui")
        return defaults.comfyui?.promptPrefix ?? "";
    if (defaults.service === "novelai")
        return defaults.novelai?.promptPrefix ?? "";
    return "";
}
function imageNegativePromptPrefixFromDefaults(defaults) {
    if (!defaults)
        return "";
    if (defaults.service === "automatic1111")
        return defaults.automatic1111?.negativePromptPrefix ?? "";
    if (defaults.service === "comfyui")
        return defaults.comfyui?.negativePromptPrefix ?? "";
    if (defaults.service === "novelai")
        return defaults.novelai?.negativePromptPrefix ?? "";
    return "";
}
function splitPromptListItems(value) {
    const parts = [];
    let current = "";
    let parenDepth = 0;
    let bracketDepth = 0;
...
function splitNaturalPromptClauses(value) {
    const parts = [];
    let current = "";
    ...
    const pushCurrent = () => {
        const clean = current.trim();
        if (clean)
            parts.push({ value: clean, startsAtBoundary });
        current = "";
    };
    for (let index = 0; index < value.length; index += 1) {
        ...
        if (!insideGroup && char === ",") {
            const startsNegativeClause = /^\s*(?:avoid|no|without|exclude|do not include|don't include)\b/i.test(value.slice(index + 1));
...
function isStandaloneNegativeListInstruction(value) {
    return /^(?:avoid|exclude|do not include|don't include)\b/i.test(value.trim());
}
function splitStandaloneNegativeInstruction(value) {
...
function findTopLevelSentenceBoundary(value) {
    let parenDepth = 0;
    let bracketDepth = 0;
    let braceDepth = 0;
    let quote = null;
...
export function mergeCompiledPromptMeta(meta, compiled) {
    const diagnostics = compiled.diagnostics;
    const removed = diagnostics.removedPositiveDuplicates.length +
...
function splitPromptFragments(value, promptMode, sourcePrompt = false) {
    ...
    if (promptMode === "natural") {
        ...
        for (const part of splitNaturalPromptClauses(normalized)) {
            ...
            if (hasAvoidInstructionPrefix(clean)) {
                ...
                const listItems = part.startsAtBoundary && isStandaloneNegativeListInstruction(clean)
...
function splitNegativePromptItems(value) {
    return splitPromptListItems(value);
}
function distillTaggedPromptSource(value) {
    const fragments = [];
    for (const raw of value.split(/\n+|(?<=[.!?])\s+/g)) {
        ...
        if (negative) {
            for (const item of splitNegativePromptItems(negative)) {
                ...
                if (cleanNegative)
                    fragments.push(`avoid ${cleanNegative}`);
            }
            sentence = stripLeadingNegativeClause(sentence);
            if (!sentence)
                continue;
        }
        if (hasAvoidInstructionPrefix(sentence))
            continue;
        ...
        const clean = sentence
            ...
            .replace(/^(?:create|generate|make|draw|depict|render)\s+(?:an?\s+)?(?:polished\s+)?(?:character\s+)?(?:avatar\s+)?(?:portrait|image|picture|illustration|scene)\s+(?:of|for)?\s*/i, "")
        ...
        for (const item of splitPromptListItems(clean)) {
            ...
            if (looksLikeNameOnly(cleanItem))
                continue;
...
function deriveTaggedSourceCues(value) {
    const cues = [];
...
function reconcileProfileSubjectTags(tags, sourceCues) {
    const genderCue = sourceCues.find((cue) => /^(?:female|male|androgynous)$/.test(cue));
    if (!tags.trim() || !genderCue)
        return tags;
...
function reconcileGenderedTag(tag, genderCue) {
...
function deriveGenderCue(text) {
    ...
    if (maleExplicit > femaleExplicit)
        return "male";
    if (femaleExplicit > maleExplicit)
        return "female";
    if (maleExplicit > 0 || femaleExplicit > 0)
        return null;
    ...
    if (malePronouns >= 2 && malePronouns > femalePronouns)
        return "male";
    if (femalePronouns >= 2 && femalePronouns > malePronouns)
        return "female";
    return null;
}
...
function fallbackTaggedSourceFragments(value) {
    return value
        .split(/\n+|(?<=[.!?])\s+/g)
        .map((fragment) => cleanPromptFragment(fragment, "tagged"))
        .filter((fragment) => {
        if (!fragment || looksLikeNameOnly(fragment))
            return false;
        if (extractNegativeFragment(fragment) || hasAvoidInstructionPrefix(fragment))
            return false;
        return shouldKeepTaggedSourceFragment(fragment);
    });
}
function deriveAgeCue(text) {
    if (/\b(?:early|mid|late)\s+(?:twenties|thirties|forties|fifties|sixties)\b/.test(text))
        return null;
    if (/\b(?:young adult|adult|middle-aged|middle aged|elderly|senior)\b/.test(text))
        return null;
    if (/\b(?:child|kid|teen|teenager|minor)\b/.test(text))
        return null;
...
function distillLabeledPromptValue(label, value) {
...
function distillVisualPhrases(value) {
    ...
    if (age?.[1])
        fragments.push(age[1]);
    if (/\btall\b/i.test(text))
        fragments.push("tall");
    if (/\bstatuesque\b/i.test(text))
        fragments.push("statuesque");
    const hair = text.match(/\b(?:her|his|their)\s+([^,.;]+?\bhair)\b/i);
    if (hair?.[1])
        fragments.push(cleanVisualPhrase(hair[1]));
    const updo = text.match(/\b(?:elegant\s+)?updo\b/i);
    if (updo?.[0])
        fragments.push(cleanVisualPhrase(updo[0]));
    ...
    if (eyes?.[1]) {
        ...
        if (eyeDescription)
            fragments.push(`${eyeDescription} eyes`);
    }
    const tagLikeEyes = text.match(/\b([a-z][a-z -]{1,30})\s+eyes?\b/i);
    if (!eyes && tagLikeEyes?.[0])
        fragments.push(cleanVisualPhrase(tagLikeEyes[0]));
...
function addIfPresent(fragments, text, pattern, fragment) {
    const match = text.match(pattern);
    if (match?.[0])
        fragments.push(fragment ?? cleanVisualPhrase(match[0]));
}
function cleanVisualPhrase(value) {
    return value
...
function looksLikeTagPhrase(value) {
    const clean = value.trim();
    if (/\b(?:is|are|was|were|has|have|favors|moves|holds|lends|perches|framing|aware)\b/i.test(clean))
        return false;
    return clean.split(/\s+/g).length <= 5;
}
function shouldKeepTaggedSourceFragment(value, requireVisualCue = false) {
    const clean = value.trim();
    if (!clean)
        return false;
    if (clean.length > 120)
        return false;
    if (/\b(?:childhood|academy|army|airship|country|refugee|business|district|background|universe|agency|determined|goal|dream|survived|moved|born|build|hoped|hope|better|spells?|uncertain|terms?|eventually|struggles?|managed|opened|enrolled|expelled|tracked)\b/i.test(clean)) {
        return false;
    }
    if (/^(?:well|right|and|or|but|yet)$/i.test(clean))
        return false;
    if (requireVisualCue && !hasVisualCue(clean))
        return false;
    if (/^(?:suitable as a chat avatar|main character|owner|sole employee)$/i.test(clean))
        return false;
    return true;
}
function hasVisualCue(value) {
...
function compactPromptFragments(fragments, compactPrompt, protectedCount = 0, maxTokens = 75) {
    if (!compactPrompt)
        return fragments;
    const separator = ", ";
    ...
    const result = [...protectedFragments];
    let tokens = estimatedPromptTokens(result, separator);
    for (const { fragment } of ranked) {
        if (isLowPriorityCompactTag(fragment) && tokens > Math.floor(maxTokens * 0.8))
            continue;
        const nextTokens = tokens + (result.length ? estimatedPromptTokenCount(separator) : 0) + estimatedPromptTokenCount(fragment);
...
function estimatedPromptTokens(fragments, separator) {
    if (fragments.length === 0)
        return 0;
    return (fragments.reduce((count, fragment) => count + estimatedPromptTokenCount(fragment), 0) +
        (fragments.length - 1) * estimatedPromptTokenCount(separator));
}
function estimatedPromptTokenCount(value) {
    const clean = value.trim();
    if (!clean)
        return 0;
...
function compactTagPriority(value) {
    const tag = value.trim().toLowerCase();
    if (/\b(?:fantasy|medieval|kingdom|castle|village|tavern|dungeon|forest|field|farm|road|market|city|urban|street|alley|temple|church|ruins?|graveyard|cave|mountain|river|lake|desert|snow|rain|storm|fog|night|dawn|morning|noon|afternoon|evening|sci-fi|scifi|cyberpunk|space|futuristic|modern|contemporary|western|victorian|steampunk|environment|landscape|scenery|location|interior|exterior)\b/.test(tag)) {
        return 1;
    }
    if (/^(?:female|male|woman|man|girl|boy|non[-\s]?binary|androgynous|genderless|human|elf|dwarf|orc|android|robot|person)$/.test(tag))
        return 0;
    if (/^(?:readable expression|clear silhouette|readable face|natural expression)$/.test(tag))
        return 7;
    ...
    if (/\b(?:photorealistic|anime|cinematic|digital painting|painterly|illustration|realistic)\b/.test(tag))
        return 6;
...
function extractNegativeFragment(fragment) {
    ...
    const negative = (splitPromptListItems(match[1])[0] ?? "")
        ...
        .replace(/^(?:any|all)\s+/i, "")
        .trim();
    if (!negative || looksLikeNonImageNegativeFragment(negative))
        return null;
    return negative;
}
function stripLeadingNegativeClause(fragment) {
    const clean = fragment.trim();
    const match = clean.match(/^(?:avoid|no|without|exclude|do not include|don't include)\s+(.+)/i);
    if (!match?.[1])
        return clean;
...
function cleanPromptFragment(fragment, promptMode) {
    let clean = fragment
        ...
        .replace(/^\s*(?:major character moment|key emotional moment|major reveal|dramatic action scene|important scene|scene moment)\s*[-:]?\s*/i, "")
        .replace(/^(?:create|generate|make|draw|depict|render)\s+(?:an?\s+)?(?:image|picture|illustration|portrait|scene)\s+(?:of|for)?\s*/i, "")
...
function dedupeFragments(fragments, strength, diagnostics) {
    const seen = new Set();
    const result = [];
...
function fragmentKey(fragment, strength) {
    ...
    if (alias)
        return alias;
    if (strength === "strict")
        return strictAlias(base) ?? base;
    return base;
}
function stripPromptWeight(value) {
...
function tagAlias(value) {
    if (/^(?:best|high|good|excellent) quality$/.test(value))
        return "quality_high";
    if (/^(?:low|bad|poor) quality$/.test(value))
        return "quality_low";
    if (/^(?:text|letters|caption|captions|subtitle|subtitles)$/.test(value))
        return "text_artifacts";
    if (/^(?:watermark|logo|signature)$/.test(value))
        return value;
    if (/^(?:blurry|blur|out of focus)$/.test(value))
        return "blurry";
    if (/^(?:solo|single subject|one subject)$/.test(value))
        return "solo_subject";
    if (/^(?:centered|centered composition|centre composition)$/.test(value))
        return "centered_composition";
    if (/^(?:centered\s+)?(?:realistic\s+)?(?:avatar\s+)?portrait$/.test(value) ||
...
function strictAlias(value) {
    if (/^(?:masterpiece|best quality|high quality|absurdres|highres)$/.test(value))
        return "quality_cluster";
    if (/^(?:portrait|avatar portrait|character portrait)$/.test(value))
        return "portrait";
    if (/^(?:upper body|bust shot|shoulders up|shoulders-up composition)$/.test(value))
        return "upper_body";
    if (/^(?:looking at viewer|facing viewer|looking toward viewer)$/.test(value))
        return "looking_at_viewer";
    return null;
}
function joinFragments(fragments, promptMode) {
    if (promptMode === "natural")
        return fragments.join(". ");
    return fragments.join(", ");
}

...
async function N(r) {
	let { id: c, version: l } = r.package, d = E(r.dataDir), f = D(r.dataDir), p = j(r.dataDir), m = M(r.dataDir), g = A(r.dataDir), C = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), N = Promise.resolve(), P = async (t, n) => {
		if (t.mode !== "roleplay" || !t.content.trim()) return;
		let i = await d.read();
		if (!i.enabled || !n && i.trackingMode === "manual") return;
		let a = n ?? i.trackingMode, o = `${t.messageId}:${t.swipeIndex}`;
		if (!n && await g.has(o)) return;
		let [s, c, l] = await Promise.all([
			r.api.runtime.persistence.getChat(t.chatId),
			r.api.runtime.persistence.listMessages(t.chatId),
			f.list()
		]);
		if (!s || s.mode !== "roleplay") return;
		let h = c.slice(-20).map((e) => ({
			role: e.role,
		...
		})), v = await (await r.api.runtime.languageModels.resolve(i.connectionId)).chatComplete([{
			role: "system",
			...
			temperature: .1,
			maxTokens: 2400
		}), y = v.content ? O(r.api.runtime.json.parseJsonish(v.content)) : [];
		if (a === "automatic") {
			let e = await f.ensureChatFolder(s.id, s.name);
			for (let t of y) {
				if (t.action === "create") {
		...
		n || await g.add(o);
	}, F = r.api.registerAssistantMessageObserver((e) => {
		N = N.then(() => P(e)).catch((e) => {
	...
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		let n = b(e.params);
		return n ? { pending: await m.list(n) } : t.status(400).send({ error: "Invalid chat id" });
		...
		try {
			...
			return !e || e.mode !== "roleplay" || !i ? t.status(400).send({ error: "This Roleplay has no assistant message to analyze yet" }) : (await P({
				chatId: n,
				messageId: i.id,
				swipeIndex: 0,
				mode: "roleplay",
				content: i.content
			}, "supervised"), { pending: await m.list(n) });
		} catch (e) {
		...
		let i = (await m.list()).find((e) => e.id === n);
		if (!i) return t.status(404).send({ error: "Pending extraction not found" });
		try {
			let e = await f.ensureChatFolder(i.chatId, i.chatName);
			...
			return await m.take(n), { accepted: !0 };
		} catch (e) {
		...
		let n = v(e.params);
		return n ? await m.take(n) ? { rejected: !0 } : t.status(404).send({ error: "Pending extraction not found" }) : t.status(400).send({ error: "Invalid pending extraction id" });
		...
		let a = (await m.list()).find((e) => e.id === i);
		if (!a) return n.status(404).send({ error: "Pending extraction not found" });
		let o = t.body && typeof t.body == "object" && !Array.isArray(t.body) ? t.body : null, s = o?.uncheckedDisposition, c = k(o?.accepted, a.updates.length);
		...
		let l = new Set(c.map((e) => e.index)), h = a.updates.filter((e, t) => !l.has(t));
		try {
			let t = [];
			if (s === "regenerate" && h.length > 0) {
				...
				t = s.content ? O(r.api.runtime.json.parseJsonish(s.content)) : [];
			}
			let o = await f.ensureChatFolder(a.chatId, a.chatName);
			for (let e of c) {
				let t = {
					...a.updates[e.index],
					appearance: e.appearance,
			...
			return await m.take(i), t.length > 0 && await m.add({
				...a,
				id: e(),
				updates: t,
				createdAt: (/* @__PURE__ */ new Date()).toISOString(),
		...
		try {
			...
			for (let t of e.deletedNpcs) {
				...
				S({ filename: e }) && await i(s(C, e), { force: !0 });
			}
		...
		try {
			await t(C, { recursive: !0 });
			let n = `${e()}.${o.image.extension}`;
			return await a(s(C, n), o.buffer, { mode: 384 }), i.status(201).send({ avatar: `/api/capabilities/npc-gallery/avatars/${n}` });
		} catch (e) {
		...
		try {
			let e = await n(s(C, i)), r = o(i).toLowerCase(), a = r === ".png" ? "image/png" : r === ".jpg" ? "image/jpeg" : r === ".gif" ? "image/gif" : "image/webp";
		...
		F(), r.api.runtime.logger.info("Deactivated capability package %s@%s", c, l);
	};
}
async function P(e) {
...
//#endregion
export { N as activate, P as selfCheck };

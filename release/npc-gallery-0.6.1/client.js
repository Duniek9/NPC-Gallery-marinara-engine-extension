...
var v = class extends HTMLElement {
	...
	renderStorySelection() {
		...
		let r = new Set(this.chatSelection.includedNpcIds), i = [...this.folders.map((e) => ({
			id: e.id,
			name: e.name,
	...
	renderGallery() {
		...
		let o = (e, n, r, i) => {
			...
			}), e !== "all" && (o.title = `Drop an NPC here to move it to ${n}`, o.addEventListener("dragover", (e) => {
	...
	render() {
		...
		this.replaceChildren(), this.style.cssText = "display: flex; min-height: 0; height: 100%; overflow: hidden";
		let t = document.createElement("section");
		...
		g.textContent = "Tracking behavior", g.style.cssText = "display: flex; flex-direction: column; gap: 0.375rem; margin-top: 1rem; font-size: 0.75rem; font-weight: 600";
		let _ = document.createElement("select");
		_.disabled = !this.settings || this.saving, _.style.cssText = "width: 100%; border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); color: var(--foreground); padding: 0.625rem 0.75rem; font-size: 0.875rem";
		for (let e of [
			{
				value: "automatic",
			...
			let t = document.createElement("option");
			t.value = e.value, t.textContent = e.label, _.append(t);
		}
		_.value = this.settings?.trackingMode ?? "supervised", _.addEventListener("change", () => void this.setTrackingMode(_.value));
		let v = document.createElement("span");
		v.textContent = "The master switch disables all tracking. These policies become active when the post-message tracker is connected.", v.style.cssText = "color: var(--muted-foreground); font-size: 0.6875rem; font-weight: 400; line-height: 1.4", g.append(_, v);
		let y = document.createElement("section");
		y.style.cssText = "display: flex; flex-direction: column; gap: 0.5rem; margin-top: 1rem";
		let b = document.createElement("div");
		b.style.cssText = "display: flex; align-items: center; justify-content: space-between; gap: 0.75rem";
		let x = document.createElement("strong");
		x.textContent = "NPC extraction prompt", x.style.cssText = "font-size: 0.75rem";
		let S = document.createElement("span");
		S.textContent = this.settings?.extractionPromptOverride ? "Customized" : "Using default", S.style.cssText = [
			"border: 1px solid var(--border)",
			"border-radius: 999px",
		...
		C.textContent = "This prompt will instruct the selected model how to identify durable NPC information. It is not executed yet.", C.style.cssText = "margin: 0; color: var(--muted-foreground); font-size: 0.6875rem; line-height: 1.4";
		let w = document.createElement("textarea");
		w.value = this.promptDraft || this.settings?.extractionPromptOverride || s, w.disabled = !this.settings || this.saving, w.maxLength = r, w.rows = 16, w.spellcheck = !1, w.style.cssText = [
			"width: 100%",
			"box-sizing: border-box",
			"resize: vertical",
			"border: 1px solid var(--border)",
			...
			"font-size: 0.75rem",
			"line-height: 1.5"
		].join(";"), w.addEventListener("input", () => {
			this.promptDraft = w.value;
		});
		let T = document.createElement("div");
		T.style.cssText = "display: flex; flex-wrap: wrap; gap: 0.5rem";
		let E = document.createElement("button");
		E.type = "button", E.textContent = this.saving ? "Saving…" : "Save prompt", E.disabled = !this.settings || this.saving, E.style.cssText = "border: 0; border-radius: 0.5rem; background: var(--primary); color: var(--primary-foreground); padding: 0.5rem 0.75rem; font-size: 0.75rem; font-weight: 600; cursor: pointer", E.addEventListener("click", () => void this.savePromptOverride());
		let D = document.createElement("button");
		D.type = "button", D.textContent = "Reset to default", D.disabled = !this.settings || this.saving || !this.settings.extractionPromptOverride, D.style.cssText = "border: 1px solid var(--border); border-radius: 0.5rem; background: var(--secondary); color: var(--foreground); padding: 0.5rem 0.75rem; font-size: 0.75rem; font-weight: 600; cursor: pointer", D.addEventListener("click", () => void this.resetPrompt()), T.append(E, D), y.append(b, C, w, T);
		let O = document.createElement("p");
		O.textContent = this.errorMessage, O.style.cssText = "margin: 0.5rem 0 0; color: var(--destructive); font-size: 0.75rem", t.append(n, i), this.activeView === "gallery" ? t.append(this.renderStorySelection(), this.renderGallery()) : t.append(a, o, u, g, y), this.deletingFolder && t.append(this.renderFolderDeleteModal()), this.errorMessage && t.append(O), this.append(t);
	}
};

...
var v = class extends HTMLElement {
	...
	async loadPendingExtractions() {
		...
		let t = await e.json();
		this.pendingExtractions = Array.isArray(t.pending) ? t.pending : [];
	}
	restoreNpcDraft() {
		try {
	...
	async analyzeRecentConversation() {
		if (!(!this.activeChatId || this.analyzing)) {
			...
			try {
				...
				let t = await e.json();
				this.pendingExtractions = Array.isArray(t.pending) ? t.pending : [], this.errorMessage = "";
			} catch (e) {
	...
	async resolvePendingExtraction(e, t) {
		try {
			...
			this.pendingExtractions = this.pendingExtractions.filter((t) => t.id !== e), t === "accept" ? await this.loadSettings() : this.render();
		} catch (e) {
	...
	renderStorySelection() {
		...
		for (let t of this.pendingExtractions) {
			let n = document.createElement("article");
			n.style.cssText = "margin-bottom: 0.75rem; border: 1px solid var(--primary); border-radius: 0.625rem; background: color-mix(in srgb, var(--primary) 8%, var(--secondary)); padding: 0.75rem";
			let r = document.createElement("strong");
			r.textContent = `${t.updates.length} NPC suggestion${t.updates.length === 1 ? "" : "s"} awaiting review`, r.style.cssText = "display: block; font-size: 0.75rem";
			let i = document.createElement("p");
			i.textContent = t.updates.map((e) => `${e.action === "create" ? "Create" : "Update"} ${e.name}`).join(" · "), i.style.cssText = "margin: 0.375rem 0; color: var(--muted-foreground); font-size: 0.6875rem; line-height: 1.4";
			let a = document.createElement("div");
			a.style.cssText = "display: flex; gap: 0.5rem";
			let o = (e, n) => {
				let r = document.createElement("button");
				return r.type = "button", r.textContent = e, r.style.cssText = `border: 1px solid var(--border); border-radius: 0.375rem; background: ${n === "accept" ? "var(--primary)" : "var(--secondary)"}; color: ${n === "accept" ? "var(--primary-foreground)" : "var(--foreground)"}; padding: 0.375rem 0.625rem; font-size: 0.6875rem; cursor: pointer`, r.addEventListener("click", () => void this.resolvePendingExtraction(t.id, n)), r;
			};
			a.append(o("Approve", "accept"), o("Reject", "reject")), n.append(r, i, a), e.append(n);
		}

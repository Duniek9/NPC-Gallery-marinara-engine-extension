...
async function k(r) {
	let { id: c, version: l } = r.package, d = T(r.dataDir), f = E(r.dataDir), p = O(r.dataDir), m = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), h = Promise.resolve(), g = r.api.registerAssistantMessageObserver((e) => {
		h = h.then(async () => {
			if (e.mode !== "roleplay" || !e.content.trim()) return;
			let t = await d.read();
			if (!t.enabled || t.trackingMode === "manual") return;
			let n = `${e.messageId}:${e.swipeIndex}`;
			if (await p.has(n)) return;
			...
			let s = a.slice(-20).map((t) => ({
				...
				temperature: .1,
				maxTokens: 2400
			}), m = l.content ? D(r.api.runtime.json.parseJsonish(l.content)) : [];
			...
			} else m.length > 0 && r.api.runtime.logger.info("NPC Gallery supervised RP extraction produced %s pending suggestion(s); review UI is the next integration step", m.length);
			await p.add(n);
		}).catch((e) => {
	...
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		try {
			...
			for (let t of e.deletedNpcs) {
				...
				x({ filename: e }) && await i(s(m, e), { force: !0 });
			}
		...
		try {
			await t(m, { recursive: !0 });
			let n = `${e()}.${o.image.extension}`;
			return await a(s(m, n), o.buffer, { mode: 384 }), i.status(201).send({ avatar: `/api/capabilities/npc-gallery/avatars/${n}` });
		} catch (e) {
		...
		try {
			let e = await n(s(m, i)), r = o(i).toLowerCase(), a = r === ".png" ? "image/png" : r === ".jpg" ? "image/jpeg" : r === ".gif" ? "image/gif" : "image/webp";
		...
		g(), r.api.runtime.logger.info("Deactivated capability package %s@%s", c, l);
	};
}
async function A(e) {
...
//#endregion
export { k as activate, A as selfCheck };

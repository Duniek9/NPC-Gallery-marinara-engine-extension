...
async function j(r) {
	let { id: c, version: l } = r.package, d = E(r.dataDir), f = D(r.dataDir), p = A(r.dataDir), m = k(r.dataDir), g = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), C = Promise.resolve(), j = r.api.registerAssistantMessageObserver((e) => {
		C = C.then(async () => {
			...
			let s = a.slice(-20).map((t) => ({
				...
				temperature: .1,
				maxTokens: 2400
			}), h = l.content ? O(r.api.runtime.json.parseJsonish(l.content)) : [];
	...
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		try {
			...
			for (let t of e.deletedNpcs) {
				...
				S({ filename: e }) && await i(s(g, e), { force: !0 });
			}
		...
		j(), r.api.runtime.logger.info("Deactivated capability package %s@%s", c, l);
	};
}
async function M(e) {
...
//#endregion
export { j as activate, M as selfCheck };

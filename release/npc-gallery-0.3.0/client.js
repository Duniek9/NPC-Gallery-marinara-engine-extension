...
var v = class extends HTMLElement {
	...
	async exportNpcCard() {
		...
		try {
			...
			c.href = u, c.download = `${e.name.toLocaleLowerCase().replace(/[^a-z0-9]+/gu, "-").replace(/^-|-$/gu, "") || "npc"}.png`, c.click(), setTimeout(() => URL.revokeObjectURL(u), 1e3), this.errorMessage = "";
		} catch (e) {
	...
	async importNpcCard(e) {
		if (!(this.draftDirty && !window.confirm("Discard the current unsaved NPC draft and import this card?"))) try {
			...
			this.npcDraft = {
				...
				sourceChatIds: [],
				lastSeenAt: null
			}, this.clearNpcDraft(), this.persistNpcDraft(!0), await this.uploadAvatar(e), this.draftRestored = !0, this.render();
		} catch (e) {

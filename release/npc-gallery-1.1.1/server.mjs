...
async function L(r) {
	let { id: c, version: l } = r.package, f = D(r.dataDir), m = O(r.dataDir), _ = F(r.dataDir), w = I(r.dataDir), L = P(r.dataDir), R = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), z = Promise.resolve(), B = async (t, n) => {
		...
		})), g = await (await r.api.runtime.languageModels.resolve(i.connectionId)).chatComplete([{
			role: "system",
		...
		if (a.length > 0) try {
			let e = await f.read(), t = await (await r.api.runtime.languageModels.resolve(e.connectionId)).chatComplete([{
				role: "system",
				content: d
			}, {
				role: "user",
				...
				temperature: .25,
				maxTokens: 1800
			}), n = t.content ? k(r.api.runtime.json.parseJsonish(t.content)) : [];
			for (let e of n) o.set(A(e.name), e);
		} catch (e) {
	...
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		try {
			...
			if (s === "regenerate" && d.length > 0) {
				...
				t = M(s.content ? k(r.api.runtime.json.parseJsonish(s.content)) : [], d, o);
			}

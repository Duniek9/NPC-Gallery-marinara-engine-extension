...
3. Enter the full Marinara Engine 2.4.1 folder when prompted. This is the folder containing Marinara's root `package.json`.
...
```bat
install-npc-gallery.bat "C:\path\to\Marinara-Engine-2.4.1"
```
...
Run the installer first on an unmodified Marinara 2.4.1 installation. The backup captures whatever files exist at installation time. Do not use an already integrated development copy as the original backup baseline.

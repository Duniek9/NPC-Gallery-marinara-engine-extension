...
import { notifyAssistantMessageCommitted } from "../services/capability-packages/assistant-message-observer.service.js";
import { collectCapabilityGenerationContext } from "../services/capability-packages/generation-context-contributor.service.js";
...
export async function generateRoutes(app: FastifyInstance) {
  ...
  app.post("/", async (req, reply) => {
    ...
    try {
      ...
      const committedAssistantMessages = new Map<string, { messageId: string; swipeIndex: number }>();
      ...
      while (true) {
        ...
        const capabilityGenerationContext = await collectCapabilityGenerationContext(chatActiveAgentIds, {
          chatId: input.chatId,
          mode: chatMode,
        });
        if (capabilityGenerationContext.length > 0) {
          finalMessages.push({
            role: "user",
            content: `<capability_memory>\n${capabilityGenerationContext.join("\n\n")}\n</capability_memory>`,
          });
        }
        ...
        const generateForCharacter = async (
          ...
          if (!input.impersonate && savedMsg?.id && savedSwipeIndex !== null) {
            committedAssistantMessages.set(`${savedMsg.id}:${savedSwipeIndex}`, {
              messageId: savedMsg.id,
              swipeIndex: savedSwipeIndex,
            });
          }
      ...
      if (!abortController.signal.aborted) {
        for (const committed of committedAssistantMessages.values()) {
          const persisted = await chats.getMessage(committed.messageId);
          if (!persisted || persisted.role !== "assistant") continue;
          notifyAssistantMessageCommitted({
            chatId: input.chatId,
            messageId: committed.messageId,
            swipeIndex: committed.swipeIndex,
            mode: chatMode,

...
interface GameSetupWizardProps {
  /** Chat being configured; capability setup views persist selection against it. */
  chatId: string;
...
export function GameSetupWizard({
  ...
  const [enableNpcGallery, setEnableNpcGallery] = useState(false);
  ...
  const npcGalleryInstalled = installedAgentIds.has("npc-gallery");
    ...
    try {
      ...
      setEnableAgents(
        config.enableAgents === true ||
          config.activeAgentIds?.includes("npc-gallery") === true ||
      ...
      setEnableNpcGallery(config.activeAgentIds?.includes("npc-gallery") === true);
    ...
    onComplete(
      {
        ...
        partyCharacterIds,
        activeAgentIds: enableAgents && enableNpcGallery ? ["npc-gallery"] : undefined,
          ...
          <>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                {enableAgents && npcGalleryInstalled && (
                  <div className="rounded-lg bg-[var(--secondary)] ring-1 ring-transparent">
                    <button
                      type="button"
                      onClick={() => setEnableNpcGallery((enabled) => !enabled)}
                      className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left"
                    >
                      <div className="min-w-0">
                        <span className="block text-xs font-medium text-[var(--foreground)]">
                          {localizeUi("ui.game.gamesetupwizard.npcGallery")}
                        </span>
                        <span className="block text-[0.575rem] text-[var(--muted-foreground)]">
                          {localizeUi("ui.game.gamesetupwizard.npcGalleryDescription")}
                        </span>
                      </div>
                      <span
                        className={cn(
                          "shrink-0 text-[0.625rem]",
                          enableNpcGallery ? "text-[var(--primary)]" : "text-[var(--muted-foreground)]",
                        )}
                      >
                        {enableNpcGallery ? "Enabled" : "Disabled"}
                      </span>
                    </button>
                    {enableNpcGallery && (
                      <div className="border-t border-[var(--border)] p-3">
                        <CapabilityElement
                          ...
                          view="setup"
                          capabilityProps={{ chatId, chatMode: "game", enabledForChat: true }}
                        />
                      </div>
                    )}
                  </div>

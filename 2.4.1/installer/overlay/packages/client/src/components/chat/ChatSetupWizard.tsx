                  ...
                  return (
                    <div key={section.category} className="border-b border-[var(--border)]/70 last:border-b-0">
                      ...
                      {section.agents.map((agent) => {
                        ...
                        const featureSettingsPackage = installedCapabilityPackages.find(
                          (item) =>
                            item.status === "active" &&
                            Boolean(item.manifest.entrypoints.client) &&
                            item.manifest.contributions?.slots?.includes("chat-settings") &&
                            item.manifest.contributions.agentDetail?.agentIds.includes(agent.id),
                        );
                        return (
                          <div key={agent.id}>
                            <button
                            ...
                            {active && featureSettingsPackage && (
                              <CapabilityElement
                                ...
                                className="mx-3 mb-2 block overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--background)]/45"
                              />
                            )}
                          </div>
                        );

...
export function ChatSettingsDrawer({
  ...
  const featureSettingsPackageByAgentId = useMemo(() => {
    ...
    for (const item of installedCapabilities) {
      ...
      if (!item.manifest.contributions?.slots?.includes("chat-settings")) continue;
      for (const agentId of item.manifest.contributions.agentDetail?.agentIds ?? []) {
        packages.set(agentId, item);
      }
    }
    return packages;
                                ...
                                {activeInCat.length > 0 && (
                                  <div className="flex flex-col gap-1 mb-1.5">
                                    {activeInCat.map((agent) => {
                                      const tokenEst = agentLoadCost.tokensByType.get(agent.id);
                                      const featureSettingsPackage = featureSettingsPackageByAgentId.get(agent.id);
                                      return (
                                        <div
                                          key={agent.id}
                                          id={
                                        ...
                                        >
                                          ...
                                          {featureSettingsPackage &&
                                            featureSettingsPackage.id !== mapsPackage?.id &&
                                            featureSettingsPackage.id !== ltmPackage?.id && (
                                              <CapabilityElement
                                                packageId={featureSettingsPackage.id}
                                                view="settings"
                                                capabilityProps={{
                                                  chatId: chat.id,
                                                  chatName: chat.name,

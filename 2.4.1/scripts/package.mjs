...
const entrypoints = [
  ...
  { source: join(projectRoot, "agents.json"), path: "agents.json", normalizeLf: true },
];
...
for (const entrypoint of entrypoints) {
  const sourceContents = await readFile(entrypoint.source);
  const contents = entrypoint.normalizeLf
    ? Buffer.from(sourceContents.toString("utf8").replace(/\r\n/g, "\n"), "utf8")
    : sourceContents;
  await writeFile(join(packageDirectory, entrypoint.path), contents);
  files.push({

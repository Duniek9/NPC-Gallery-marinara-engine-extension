...
const LAUNCHER_HOOKS = [
  {
    name: "start.bat",
    marker: ":: NPC Gallery updater bootstrap begin",
    anchor: ":skip_env",
    lines: [
      ":: NPC Gallery updater bootstrap begin",
      ":: Reconcile after Engine updates and before build/start.",
  ...
  },
  {
    name: "start.sh",
    marker: "# NPC Gallery updater bootstrap begin",
    anchor: "# ── Optional AI sprite background remover ──",
    lines: POSIX_LAUNCHER_HOOK,
  },
  {
    name: "start-termux.sh",
    marker: "# NPC Gallery updater bootstrap begin",
    anchor: "# ── Build if needed ──",
    lines: POSIX_LAUNCHER_HOOK,
  },
];
...
async function ensureLauncherHooks(engineRoot) {
  for (const specification of LAUNCHER_HOOKS) {
    ...
    try {
      ...
      if (contents.includes(specification.marker)) continue;
      const anchorIndex = contents.indexOf(specification.anchor);
      if (anchorIndex < 0) {
      ...
      const hook = `${specification.lines.join(endOfLine)}${endOfLine}${endOfLine}`;
      const updated = `${contents.slice(0, anchorIndex)}${hook}${contents.slice(anchorIndex)}`;
      const temporary = `${launcherPath}.npc-gallery-${process.pid}-${Date.now()}`;
      const launcherStat = await stat(launcherPath);
      try {
        await writeFile(temporary, updated, { encoding: "utf8", mode: launcherStat.mode });
        await rename(temporary, launcherPath);
      } finally {
        await rm(temporary, { force: true });
      }
      log(`Restored the persistent updater hook in ${specification.name}.`);
    } catch (error) {

...
Starting with NPC Gallery 1.3.0, a persistent pre-start updater checks exact Marinara compatibility after Engine updates. Version 1.3.1 adds a temporary launcher wrapper so that check survives Marinara replacing its launcher during the update itself. The updater installs a verified matching bundle when available, or suspends only NPC Gallery when no compatible bundle exists. Saved NPC data is always preserved.
...
Linux and Termux installers are maintained in the newest `2.4.6` edition only:

- Linux: `2.4.6/install-npc-gallery-linux.sh`
- Termux: `2.4.6/install-npc-gallery-termux.sh`

Use the matching uninstall script from the same folder to restore Marinara's original files. See [`2.4.6/INSTALLING.md`](2.4.6/INSTALLING.md) for commands, backup behavior, Termux's low-memory build, and automatic updater behavior.

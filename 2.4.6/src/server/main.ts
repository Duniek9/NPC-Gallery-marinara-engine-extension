...
interface ActivationContext {
  ...
  api: {
    runtime: {
      ...
      languageModels: {
        resolveForRequest(input: { connectionId?: string | null; chatConnectionId?: string | null }): Promise<{
      ...
      persistence: {
        getChat(chatId: string): Promise<{
          id: string; name: string; mode: string; connectionId?: string | null; characterIds?: string[]; metadata?: string | Record<string, unknown> | null;
        } | null>;
        listMessages(chatId: string): Promise<Array<{ id: string; role: string; content: string }>>;
      };
    };
    registerService<T>(key: string, service: T): () => void;
    registerPromptContext(contributor: (event: PromptContextEvent) => string | null | Promise<string | null>): () => void;
  };
}
...
interface GameSetupContextEvent {
  chatId: string;
  chatName: string;
  partyCharacterNames: string[];
}

interface GameSetupCompletedEvent extends GameSetupContextEvent {
  startingNpcs: Array<Record<string, unknown>>;
}

interface PromptContextEvent {
  chatId: string;
  chatMeta: Record<string, unknown>;
  mode: string;
}

interface NpcGalleryIntegrationService {
  onAssistantMessageCommitted(event: AssistantMessageCommittedEvent): void;
  getGameSetupContext(event: GameSetupContextEvent): Promise<string | null>;
  onGameSetupCompleted(event: GameSetupCompletedEvent): Promise<void>;
}
...
export async function activate(context: ActivationContext): Promise<() => void> {
  ...
  const analyzeEvent = async (event: AssistantMessageCommittedEvent, forcedMode?: "supervised") => {
      if (
        (event.mode !== "roleplay" && event.mode !== "game" && event.mode !== "conversation") ||
        !event.content.trim()
      ) return;
      ...
      if (!chat || (chat.mode !== "roleplay" && chat.mode !== "game" && chat.mode !== "conversation")) return;
      ...
      const model = await context.api.runtime.languageModels.resolveForRequest({
  ...
  const queueAssistantMessage = (event: AssistantMessageCommittedEvent): void => {
  ...
  const getGameSetupContext = async (event: GameSetupContextEvent): Promise<string | null> => {
  ...
  const unregisterPromptContext = context.api.registerPromptContext(async (event) => {
    if (event.mode !== "roleplay" && event.mode !== "game" && event.mode !== "conversation") return null;
    if (event.chatMeta.enableAgents !== true) return null;
    if (!Array.isArray(event.chatMeta.activeAgentIds) || !event.chatMeta.activeAgentIds.includes("npc-gallery")) {
      return null;
    }
    const selection = await chatSelections.get(event.chatId);
    if (selection.includedNpcIds.length === 0) return null;
  ...
  const handleGameSetupCompleted = async (event: GameSetupCompletedEvent): Promise<void> => {
    ...
    if (missingDetails.length > 0) {
      try {
        const [currentSettings, chat] = await Promise.all([
          settings.read(),
          context.api.runtime.persistence.getChat(event.chatId),
        ]);
        const model = await context.api.runtime.languageModels.resolveForRequest({
  ...
  const integrationService: NpcGalleryIntegrationService = {
    onAssistantMessageCommitted: queueAssistantMessage,
    getGameSetupContext,
    onGameSetupCompleted: handleGameSetupCompleted,
  };
  const unregisterIntegrationService = context.api.registerService("npc-gallery:integration", integrationService);
  ...
  context.app.post("/api/capabilities/npc-gallery/analyze/:id", async (request, reply) => {
    ...
    try {
      ...
      if (
        !chat ||
        (chat.mode !== "roleplay" && chat.mode !== "game" && chat.mode !== "conversation") ||
        !message
      ) {
  ...
  context.app.post("/api/capabilities/npc-gallery/pending/:id/review", async (request, reply) => {
    ...
    try {
      ...
      if (disposition === "regenerate" && unchecked.length > 0) {
        const [settingsValue, chat, messages, existingNpcs] = await Promise.all([
          settings.read(),
          context.api.runtime.persistence.getChat(pending.chatId),
        ...
        const model = await context.api.runtime.languageModels.resolveForRequest({
          connectionId: settingsValue.connectionId,
          chatConnectionId: chat?.connectionId,
        });
        const completion = await model.chatComplete([

...
interface NpcGalleryIntegrationService {
  onAssistantMessageCommitted(event: {
    chatId: string;
    messageId: string;
    swipeIndex: number;
    mode: string;
  ...
  app.post("/", async (req, reply) => {
    ...
    try {
      ...
      if (
        !abortController.signal.aborted &&
        chatEnableAgents &&
        chatActiveAgentIds.includes("npc-gallery")
      ) {
        const npcGallery = getCapabilityService<NpcGalleryIntegrationService>("npc-gallery:integration");
        if (npcGallery) {
          for (const committed of committedAssistantMessages.values()) {
            ...
            npcGallery.onAssistantMessageCommitted({
              chatId: input.chatId,

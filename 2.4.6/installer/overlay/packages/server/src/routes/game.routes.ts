...
interface NpcGalleryIntegrationService {
  getGameSetupContext(event: {
    chatId: string;
    chatName: string;
    partyCharacterNames: string[];
  }): Promise<string | null>;
  onGameSetupCompleted(event: {
    chatId: string;
    chatName: string;
    partyCharacterNames: string[];
...
async function notifyNpcGalleryGameSetupCompleted(event: {
  chatId: string;
  chatName: string;
  partyCharacterNames: string[];
  startingNpcs: Array<Record<string, unknown>>;
  ...
  try {
    await getCapabilityService<NpcGalleryIntegrationService>("npc-gallery:integration")?.onGameSetupCompleted(event);
  } catch (error) {
    logger.warn(error, "[game/setup] NPC Gallery starting-NPC import failed");
  }
}
...
const gameSetupConfigSchema = z.object({
  ...
  activeAgentIds: z.array(z.string().min(1).max(200)).max(100).optional(),
...
export async function gameRoutes(app: FastifyInstance) {
  ...
  app.post("/create", async (req) => {
    ...
    const setupActiveAgentIds = Array.from(new Set([
      ...(setupConfig.activeAgentIds ?? []),
  ...
  app.post("/setup", async (req, reply) => {
    ...
    let npcGallerySetupContext: string | null = null;
    const setupActiveAgentIds = Array.isArray(meta.activeAgentIds)
      ? meta.activeAgentIds.filter((value): value is string => typeof value === "string")
      : [];
    if (setupActiveAgentIds.includes("npc-gallery")) {
      try {
        npcGallerySetupContext =
          (await getCapabilityService<NpcGalleryIntegrationService>("npc-gallery:integration")?.getGameSetupContext({
            chatId,
            chatName: chat.name,
            partyCharacterNames: partyNames,
          })) ?? null;
      } catch (error) {
        logger.warn(error, "[game/setup] NPC Gallery setup context failed");
      }
    }
    const setupWorldContext = [setupLorebookContext, npcGallerySetupContext]
    ...
    const messages: ChatMessage[] = [
      {
        ...
        content: buildSetupPrompt({
          ...
          customHudWidgets: customHudWidgets.length > 0 ? customHudWidgets : undefined,
          lorebookContext: setupWorldContext || undefined,
    ...
    if (setupActiveAgentIds.includes("npc-gallery")) {
      const startingNpcs = Array.isArray(setupData.startingNpcs)
        ? setupData.startingNpcs.filter(
            (value): value is Record<string, unknown> => Boolean(value) && typeof value === "object" && !Array.isArray(value),
          )
        : [];
      await notifyNpcGalleryGameSetupCompleted({
        chatId,
        chatName: chat.name,
        partyCharacterNames: partyNames,
        startingNpcs,
  ...
  app.post("/setup/apply-json", async (req, reply) => {
    ...
    if (setupActiveAgentIds.includes("npc-gallery")) {
      ...
      const characters = createCharactersStorage(app.db);
      for (const characterId of setupConfig.partyCharacterIds) {
        ...
        try {
          ...
          if (typeof data?.name === "string" && data.name.trim()) partyCharacterNames.push(data.name.trim());
        } catch {
          // A malformed party card must not block the repaired setup from completing.
        }
      }
      const startingNpcs = Array.isArray(setupData.startingNpcs)
        ? setupData.startingNpcs.filter(

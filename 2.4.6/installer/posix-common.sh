...
REQUIRED_ENGINE_VERSION="2.4.6"
INTEGRATION_VERSION="1.3.1"
BACKUP_FOLDER_NAME=".npc-gallery-engine-backups/2.4.6"
...
require_node() {
  ...
  if ((node_major < 24 || node_major >= 27)); then
    die "Marinara Engine 2.4.6 requires Node.js 24, 25, or 26. Found $(node --version)."
  fi
}
...
build_shared() {
  ...
  die "Shared compilation failed and produced no Marinara 2.4.6 runtime output."
}

...
$RequiredEngineVersion = "2.4.6"
$IntegrationVersion = "1.3.1"
$BackupFolderName = ".npc-gallery-engine-backups/2.4.6"
    ...
    try {
      ...
      try {
        ...
        if (Test-Path -LiteralPath $SharedEntry -PathType Leaf) {
          ...
          throw "Shared compilation failed and produced no Marinara 2.4.6 runtime output."
        }
      }
    }
  }
...
if (-not (Test-Path -LiteralPath $ManifestPath -PathType Leaf)) {
  ...
  foreach ($OverlayFile in $OverlayFiles) {
    $RelativePath = $OverlayFile.FullName.Substring($OverlayRoot.Length).TrimStart('\').Replace('\', '/')
...
$UpdaterSourceRoot = Join-Path $ExtensionRoot "updater"
$UpdaterSource = Join-Path $UpdaterSourceRoot "reconcile.mjs"
$LauncherWrapperSource = Join-Path $UpdaterSourceRoot "launch-wrapper.mjs"
$UpdaterRoot = Join-Path $DataDirectory "capability-packages\bootstrap\npc-gallery"
$UpdaterDestination = Join-Path $UpdaterRoot "reconcile.mjs"
$LauncherWrapperDestination = Join-Path $UpdaterRoot "launch-wrapper.mjs"
$UpdaterStatePath = Join-Path $UpdaterRoot "state.json"

if (-not (Test-Path -LiteralPath $UpdaterSource -PathType Leaf)) {
  throw "NPC Gallery updater is missing: $UpdaterSource"
}
if (-not (Test-Path -LiteralPath $LauncherWrapperSource -PathType Leaf)) {
  throw "NPC Gallery launcher wrapper is missing: $LauncherWrapperSource"
}
...
Copy-Item -LiteralPath $LauncherWrapperSource -Destination $LauncherWrapperDestination -Force

$UpdaterState = [pscustomobject]@{
  schemaVersion     = 1
  status            = "active"
  engineVersion     = $RequiredEngineVersion
  extensionVersion  = $IntegrationVersion
...
[System.IO.File]::WriteAllText(
  $UpdaterStatePath,
  ($UpdaterState | ConvertTo-Json -Depth 5) + [Environment]::NewLine,
  (New-Object System.Text.UTF8Encoding($false))
)

Write-Host "Installed the persistent NPC Gallery updater bootstrap."

Write-Host ""
Write-Host "Installing the packaged NPC Gallery capability..."

$ReleaseValidator = Join-Path $PSScriptRoot "posix-files.mjs"
$PackageInstaller = Join-Path $ExtensionRoot "scripts\install-dev.mjs"

& node $ReleaseValidator validate-release $ExtensionRoot $IntegrationVersion
if ($LASTEXITCODE -ne 0) {
  throw "The packaged NPC Gallery release failed validation with exit code $LASTEXITCODE."
}

& node $PackageInstaller --data-dir $DataDirectory
if ($LASTEXITCODE -ne 0) {
  throw "NPC Gallery package installation failed with exit code $LASTEXITCODE."
}

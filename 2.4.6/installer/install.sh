...
main() {
  ...
  local launcher_wrapper_source="$EXTENSION_ROOT/updater/launch-wrapper.mjs"
  ...
  [[ -f "$launcher_wrapper_source" ]] || die "NPC Gallery launcher wrapper is missing: $launcher_wrapper_source"
  mkdir -p "$updater_root"
  cp -f "$updater_source" "$updater_root/reconcile.mjs"
  cp -f "$launcher_wrapper_source" "$updater_root/launch-wrapper.mjs"
  node -e '

...
async function L(r) {
	let { id: c, version: l } = r.package, f = D(r.dataDir), m = O(r.dataDir), _ = F(r.dataDir), w = I(r.dataDir), L = P(r.dataDir), R = s(r.dataDir, "capability-packages", "state", "npc-gallery", "avatars"), z = Promise.resolve(), B = async (t, n) => {
		if (t.mode !== "roleplay" && t.mode !== "game" && t.mode !== "conversation" || !t.content.trim()) return;
		let i = await f.read();
		...
		if (!s || s.mode !== "roleplay" && s.mode !== "game" && s.mode !== "conversation") return;
		let d = [];
		if (s.mode === "game") {
			let e = {};
			try {
	...
	}, U = r.api.registerPromptContext(async (e) => {
		if (e.mode !== "roleplay" && e.mode !== "game" && e.mode !== "conversation" || e.chatMeta.enableAgents !== !0 || !Array.isArray(e.chatMeta.activeAgentIds) || !e.chatMeta.activeAgentIds.includes("npc-gallery")) return null;
		let t = await _.get(e.chatId);
		if (t.includedNpcIds.length === 0) return null;
		let n = (await m.list()).filter((e) => t.includedNpcIds.includes(e.id));
		...
		onAssistantMessageCommitted: V,
		getGameSetupContext: H,
		onGameSetupCompleted: async (e) => {
	...
	}, G = r.api.registerService("npc-gallery:integration", W);
	return r.app.get("/api/capabilities/npc-gallery/settings", async (e, t) => {
		...
		try {
			...
			return !e || e.mode !== "roleplay" && e.mode !== "game" && e.mode !== "conversation" || !i ? t.status(400).send({ error: "This chat has no assistant message to analyze yet" }) : (await B({
				chatId: n,
				messageId: i.id,
				swipeIndex: 0,
				mode: e.mode,
		...
		G(), U(), r.api.runtime.logger.info("Deactivated capability package %s@%s", c, l);
	};
}
async function R(e) {

...
var v = class extends HTMLElement {
	...
	connectedCallback() {
		this.addEventListener("marinara-capability-props", this.handleCapabilityProps), this.handleCapabilityProps(), this.restoreNpcDraft(), this.render(), this.loadSettings(), (this.getAttribute("view") === "runtime" || this.getAttribute("view") === "surface") && (this.pendingPoll = window.setInterval(() => void this.pollPendingExtractions(), 2e3));
	}
	disconnectedCallback() {
		this.removeEventListener("marinara-capability-props", this.handleCapabilityProps), this.reviewModalElement?.remove(), this.reviewModalElement = null, this.pendingPoll !== null && window.clearInterval(this.pendingPoll), this.pendingPoll = null;
	}
	async loadChatSelection() {
		...
		try {
			...
			this.chatSelection = await e.json(), await this.loadPendingExtractions();
		} catch (e) {
			this.errorMessage = e instanceof Error ? e.message : "Could not load story NPC selection";
		}
		this.render();
	}
	async loadPendingExtractions() {
		if (!this.activeChatId) return;
		let e = await fetch(`/api/capabilities/npc-gallery/pending/${this.activeChatId}`, { cache: "no-store" });
		if (!e.ok) throw Error(`Pending extraction request failed with status ${e.status}`);
		let t = await e.json();
		this.pendingExtractions = Array.isArray(t.pending) ? t.pending : [], this.pendingExtractions.length > 0 && (this.reviewModalOpen = !0);
	}
	async pollPendingExtractions() {
		if (!this.activeChatId) return;
		let e = this.pendingExtractions.map((e) => e.id).join(",");
		try {
			await this.loadPendingExtractions(), this.pendingExtractions.map((e) => e.id).join(",") !== e && this.render();
		} catch {}
	}
	restoreNpcDraft() {
		try {
	...
	async analyzeRecentConversation() {
		if (!(!this.activeChatId || this.analyzing)) {
			this.analyzing = !0, this.render();
			try {
				let e = await fetch(`/api/capabilities/npc-gallery/analyze/${this.activeChatId}`, {
					method: "POST",
					headers: { "x-marinara-csrf": "1" }
				});
				if (!e.ok) {
					let t = await e.json().catch(() => ({}));
					throw Error(t.error ?? `Analysis failed with status ${e.status}`);
				}
				let t = await e.json();
				this.pendingExtractions = Array.isArray(t.pending) ? t.pending : [], this.pendingExtractions.length > 0 && (this.reviewModalOpen = !0), this.errorMessage = "";
			} catch (e) {
				this.errorMessage = e instanceof Error ? e.message : "Could not analyze recent conversation";
			} finally {
				this.analyzing = !1, this.render();
			}
		}
	}
	async resolvePendingExtraction(e, t) {
		try {
			let n = await fetch(`/api/capabilities/npc-gallery/pending/${e}/${t}`, {
				method: "POST",
				headers: { "x-marinara-csrf": "1" }
			});
			if (!n.ok) throw Error(`${t === "accept" ? "Approval" : "Rejection"} failed with status ${n.status}`);
			this.pendingExtractions = this.pendingExtractions.filter((t) => t.id !== e), this.reviewModalOpen = this.pendingExtractions.length > 0, t === "accept" ? await this.loadSettings() : this.render();
		} catch (e) {
			this.errorMessage = e instanceof Error ? e.message : "Could not resolve extraction", this.render();
		}
	}
	async submitReviewedExtraction(e, t, n) {
		if (!this.resolvingReview) {
			this.resolvingReview = !0;
			try {
				let r = await fetch(`/api/capabilities/npc-gallery/pending/${e.id}/review`, {
					method: "POST",
					headers: {
						"content-type": "application/json",
						"x-marinara-csrf": "1"
					...
					body: JSON.stringify({
						accepted: t,
						uncheckedDisposition: n
					})
				}), i = await r.json().catch(() => ({}));
				if (!r.ok) throw Error(i.error ?? `Review failed with status ${r.status}`);
				this.pendingExtractions = Array.isArray(i.pending) ? i.pending : [], this.reviewModalOpen = this.pendingExtractions.length > 0, await this.loadSettings();
			} catch (e) {
				this.errorMessage = e instanceof Error ? e.message : "Could not save reviewed NPCs", this.render();
			} finally {
				this.resolvingReview = !1;
			}
		}
	}
	renderStorySelection() {
		...
		if (this.settings?.trackingMode === "manual") {
			...
			t.type = "button", t.textContent = this.analyzing ? "Analyzing…" : "Analyze recent conversation", t.disabled = this.analyzing, t.style.cssText = "margin-bottom: 0.75rem; border: 1px solid var(--primary); border-radius: 0.5rem; background: var(--primary); color: var(--primary-foreground); padding: 0.5rem 0.75rem; font-size: 0.6875rem; font-weight: 600; cursor: pointer", t.addEventListener("click", () => void this.analyzeRecentConversation()), e.append(t);
		}
		if (this.pendingExtractions.length > 0) {
			let t = document.createElement("button");
			t.type = "button", t.textContent = `Review ${this.pendingExtractions.length} pending extraction${this.pendingExtractions.length === 1 ? "" : "s"}`, t.style.cssText = "margin-bottom: 0.75rem; border: 1px solid var(--primary); border-radius: 0.5rem; background: color-mix(in srgb, var(--primary) 12%, var(--secondary)); color: var(--foreground); padding: 0.5rem 0.75rem; font-size: 0.6875rem; font-weight: 600; cursor: pointer", t.addEventListener("click", () => {
				this.reviewModalOpen = !0, this.render();
			}), e.append(t);
		}
	...
	renderReviewModal() {
		let e = this.pendingExtractions[0], t = document.createElement("div");
		t.style.cssText = "position: fixed; inset: 0; z-index: 10040; display: flex; align-items: center; justify-content: center; background: rgb(0 0 0 / 0.72); padding: 1rem";
		let n = document.createElement("section");
		n.setAttribute("role", "dialog"), n.setAttribute("aria-modal", "true"), n.style.cssText = "width: min(42rem, 100%); max-height: min(44rem, calc(100vh - 2rem)); overflow-y: auto; border: 1px solid var(--border); border-radius: 0.875rem; background: var(--background); color: var(--foreground); padding: 1rem; box-shadow: 0 1.5rem 4rem rgb(0 0 0 / 0.45)";
		let r = document.createElement("strong");
		r.textContent = `NPC changes need approval (${this.pendingExtractions.length} pending)`, r.style.cssText = "display: block; font-size: 1rem";
		let i = document.createElement("p");
		i.textContent = `Review suggestions detected in ${e.chatName}. Nothing is saved until you approve.`, i.style.cssText = "margin: 0.375rem 0 0.75rem; color: var(--muted-foreground); font-size: 0.75rem", n.append(r, i);
		let a = [];
		e.updates.forEach((e) => {
			let t = document.createElement("article");
			t.style.cssText = "margin-top: 0.625rem; border: 1px solid var(--border); border-radius: 0.625rem; background: var(--card); padding: 0.75rem";
			let r = document.createElement("label");
			r.style.cssText = "display: flex; align-items: center; gap: 0.5rem; font-size: 0.8125rem; font-weight: 600; cursor: pointer";
			let i = document.createElement("input");
			i.type = "checkbox", i.checked = !0, r.append(i, `${e.action === "create" ? "Create" : "Update"}: ${e.name}`);
			let o = document.createElement("div");
			o.style.cssText = "margin-top: 0.625rem; display: grid; gap: 0.5rem";
			let s = (e, t, n) => {
				let r = document.createElement("label");
				r.style.cssText = "display: grid; gap: 0.25rem; color: var(--muted-foreground); font-size: 0.6875rem", r.append(e);
				let i = document.createElement("textarea");
				return i.rows = n, i.value = t ?? "", i.style.cssText = "width: 100%; resize: vertical; border: 1px solid var(--border); border-radius: 0.45rem; background: var(--input); color: var(--foreground); padding: 0.5rem; font: inherit; box-sizing: border-box", r.append(i), o.append(r), i;
			}, c = s("Appearance", e.appearance, 3), l = s("Personality", e.personality, 3), u = s("Description", e.description, 4);
			i.addEventListener("change", () => {
				o.style.opacity = i.checked ? "1" : "0.5";
			}), a.push({
				checked: i,
				appearance: c,
				personality: l,
				description: u
			}), t.append(r, o), n.append(t);
		});
		let o = document.createElement("div");
		o.style.cssText = "position: sticky; bottom: -1rem; display: flex; flex-wrap: wrap; gap: 0.5rem; margin: 1rem -1rem -1rem; border-top: 1px solid var(--border); background: var(--background); padding: 0.75rem 1rem";
		let s = (e, t = !1) => {
			let n = document.createElement("button");
			return n.type = "button", n.textContent = e, n.style.cssText = `border: 1px solid var(--border); border-radius: 0.5rem; background: ${t ? "var(--primary)" : "var(--secondary)"}; color: ${t ? "var(--primary-foreground)" : "var(--foreground)"}; padding: 0.5rem 0.75rem; font-size: 0.75rem; font-weight: 600; cursor: pointer`, n;
		}, c = () => {
			if (this.resolvingReview) return;
			let e = document.createElement("strong");
			e.textContent = "Regenerating NPC card…", e.style.cssText = "display: block; font-size: 1rem";
			let t = document.createElement("p");
			t.setAttribute("role", "status"), t.textContent = "Please wait—your NPC card is being regenerated.", t.style.cssText = "margin: 0.5rem 0 0; color: var(--muted-foreground); font-size: 0.75rem", n.replaceChildren(e, t);
		}, l = s("Accept checked", !0), u = s("Review later");
		if (u.addEventListener("click", () => {
			this.reviewModalOpen = !1, this.render();
		}), l.addEventListener("click", () => {
			let t = a.flatMap((e, t) => e.checked.checked ? [{
				index: t,
				appearance: e.appearance.value.trim() || null,
				personality: e.personality.value.trim() || null,
				description: e.description.value.trim() || null
			}] : []), r = a.length - t.length;
			if (r === 0) {
				this.submitReviewedExtraction(e, t, "abandon");
				return;
			}
			n.replaceChildren();
			let i = document.createElement("strong");
			i.textContent = `What do you want to do with ${r === 1 ? "this NPC" : `these ${r} NPCs`}?`;
			let o = document.createElement("p");
			o.textContent = "Checked NPCs will be saved. Choose what happens to the unchecked suggestions.", o.style.cssText = "color: var(--muted-foreground); font-size: 0.75rem";
			let l = document.createElement("div");
			l.style.cssText = "display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 1rem";
			let u = s("Regenerate unchecked", !0), d = s("Abandon unchecked"), f = s("Go back");
			u.addEventListener("click", () => {
				this.resolvingReview || (c(), this.submitReviewedExtraction(e, t, "regenerate"));
			}), d.addEventListener("click", () => void this.submitReviewedExtraction(e, t, "abandon")), f.addEventListener("click", () => this.render()), l.append(u, d, f), n.append(i, o, l);
		}), o.append(l), (e.regenerationCount ?? 0) > 0) {
			let t = s("Regenerate again");
			t.addEventListener("click", () => {
				this.resolvingReview || (c(), this.submitReviewedExtraction(e, [], "regenerate"));
			}), o.append(t);
		}
		return o.append(u), n.append(o), t.append(n), t;
	}
	renderGallery() {
	...
	render() {
		...
		this.reviewModalElement?.remove(), this.reviewModalElement = null, this.replaceChildren();
		...
		].join(";"), e === "runtime" || e === "surface") {
			n.hidden = !0, this.append(n), this.reviewModalOpen && this.pendingExtractions.length > 0 && (this.reviewModalElement = this.renderReviewModal(), document.body.append(this.reviewModalElement));
			return;
		}
		if (t) {
			if (n.style.cssText = [
			...
			this.append(n), this.reviewModalOpen && this.pendingExtractions.length > 0 && (this.reviewModalElement = this.renderReviewModal(), document.body.append(this.reviewModalElement));
			return;
		}
		let i = document.createElement("strong");
		...
		k.textContent = this.errorMessage, k.style.cssText = "margin: 0.5rem 0 0; color: var(--destructive); font-size: 0.75rem", n.append(i, a), this.activeView === "gallery" ? n.append(this.renderStorySelection(), this.renderGallery()) : n.append(o, c, d, _, b), this.deletingFolder && n.append(this.renderFolderDeleteModal()), this.errorMessage && n.append(k), this.append(n), this.reviewModalOpen && this.pendingExtractions.length > 0 && (this.reviewModalElement = this.renderReviewModal(), document.body.append(this.reviewModalElement));
	}
};

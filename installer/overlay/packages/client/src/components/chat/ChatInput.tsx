...
export const ChatInput = memo(function ChatInput({
  ...
  const runtimeCapabilityPackages = useMemo(
    () =>
      installedCapabilities.filter(
        (item) =>
          item.status === "active" &&
          Boolean(item.manifest.entrypoints.client) &&
          item.id !== "hierarchical-maps" &&
          item.manifest.contributions?.slots?.includes("chat-runtime") &&
          activeAgentIds.includes(item.id),
      ),
    [activeAgentIds, installedCapabilities],
  );
  ...
  return (
    <div className="mari-chat-input chat-input-container px-3 pb-3">
      ...
      {activeChatId &&
        runtimeCapabilityPackages.map((item) => (
          <CapabilityElement
            key={`${item.id}-runtime-${activeChatId}`}
            packageId={item.id}
            view="runtime"
            capabilityProps={{ chatId: activeChatId, chatMode: mode, enabledForChat: true }}
            className="contents"
          />
        ))}

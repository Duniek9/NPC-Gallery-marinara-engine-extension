...
const LEADING_CONVERSATION_TIMESTAMPS_RE = new RegExp(String.raw `^([^\S\n]*(?:${CONVERSATION_TIMESTAMP_TOKEN_SOURCE})\s*)+`, "gm");
 ...
 */
export function stripLeadingMessageTimestamps(text) {
...
export function parseSpeakerTags(content, knownNames) {
    ...
    if (lastIndex < decodedContent.length) {
        ...
        if (after)
            segments.push({ speaker: null, text: after, start: lastIndex, end: decodedContent.length });
    }
...
export function groupConsecutiveSegments(segments) {
    const groups = [];
 ...
 */
export function parseGroupedSpeakerSegments(content, knownNames, leadingSpeaker) {
    const speakerSegs = parseSpeakerTags(content, knownNames);
    if (speakerSegs)
        return groupConsecutiveSegments(speakerSegs);
    const nameSegs = parseNamePrefixFormat(content, knownNames, leadingSpeaker);
    if (nameSegs)
        return groupConsecutiveSegments(nameSegs);
    return null;
}
//# sourceMappingURL=speaker-segments.js.map
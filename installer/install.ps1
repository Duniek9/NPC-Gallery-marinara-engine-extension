    ...
    try {
      ...
      try {
        ...
        if (Test-Path -LiteralPath $SharedEntry -PathType Leaf) {
          ...
          if (Test-Path -LiteralPath $SharedDist) {
            Remove-Item -LiteralPath $SharedDist -Recurse -Force
          }

          New-Item -ItemType Directory -Path $SharedDist -Force |
            Out-Null

          Copy-Item `
            -Path (Join-Path $PrebuiltShared "*") `
            -Destination $SharedDist `
            -Recurse `
            -Force

          Write-Warning "Shared compilation produced no runtime output. Installed the bundled Marinara 2.3.4 Shared runtime instead."
        }
      }
    }
  }
